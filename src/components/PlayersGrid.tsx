...
import { getConversationId } from '../lib/storage/localStore'
import { createPortal } from 'react-dom'
import { analyzeBlocking } from '../lib/api'
import { toSlimPlayer } from '../lib/players/slim'
import { ActionButtonsCell } from './grid/ActionButtonsCell'
import { useDraftInitializeStream } from '../hooks/useDraftInitializeStream'
...
export const PlayersGrid: React.FC<PlayersGridProps> = ({ toast }) => {
  ...
  const markInitialized = useDraftStore((s) => s.markInitialized)
  const openAssistantStreaming = useDraftStore((s) => s.openAssistantStreaming)
  const appendAssistantStream = useDraftStore((s) => s.appendAssistantStream)
  const finishAssistantStreaming = useDraftStore((s) => s.finishAssistantStreaming)
  const failAssistantStreaming = useDraftStore((s) => s.failAssistantStreaming)
  ...
  // Suppress TS6133 for unused variables
  ...
  const hasInitializedDraft = useDraftStore((s) => s.hasInitializedDraft)
  ...
  const { start } = useDraftInitializeStream()
  ...
  // Handle initialize draft button click
  const handleInitializeDraft = React.useCallback(async () => {
    // Prepare payload (reuse logic from original DraftConfigModal)
    const numTeams = draftConfig.teams || 12;
    const userPickPosition = draftConfig.pick || 1;
    ...
    const payload = {
      ...
      payload: {
        ...
        players: top25Players
      }
    };
    
    // 3. Use setTimeout to ensure drawer layout completes before streaming
    setTimeout(() => startStreaming(payload), 0);
  }, [draftConfig.teams, draftConfig.pick, filteredData])

  const startStreaming = React.useCallback(async (payload: { user: string; payload: { numTeams: number; userPickPosition: number; players: Player[] } }) => {
    try {
      ...
      await start(payload, {
        ...
        onError: (err) => {
          ...
          failAssistantStreaming(err instanceof Error ? err.message : String(err));
          setIsInitializing(false);
        }
      });
    } catch (error) {
      if (import.meta.env.DEV) {
        console.error('[init-stream] start error', error);
      }
      setIsInitializing(false);
    }
  }, [start, appendAssistantStream, finishAssistantStreaming, markInitialized, failAssistantStreaming])
    ...
    <section className="space-y-4">
      ...
      <div style={{
        ...
        <div style={{
          ...
          {/* Conditional Button - Initialize Draft vs Analyze */}
          {!hasInitializedDraft ? (
            <Button
              ...
              tooltip={isAnalyzing ? "Analysis in progress..." : "Initialize draft to begin"}
